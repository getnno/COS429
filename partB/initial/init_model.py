...
def init_model(layers, input_size, output_size, display):
    ...
    # Batch sizes of 1 and greater than 1 are used to ensure that both cases
    # are handled properly by the code
    ...
    if display:
        ...
        for i in range(num_layers - 1):
            ...
            if len(act_size_1) == len(
                    act_size_5) and act_size_1[:-1] == act_size_5[:-1]:
                print(act_size_1[:-1])
            else:
                print(
    ...
    if isinstance(output_size, (int, float)):
        assert len(
            network_output_size) == 1, 'Network output does not match up with provided output size'
        ...
        assert list(network_output_size) == list(
            output_size), 'Network output does not match up with provided output size'
