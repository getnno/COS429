...
def fn_pool(input, params, hyper_params, backprop, dv_output=None):
    ...
    if 'pad' in hyper_params.keys():
        ...
        input = np.pad(input, ((pad, pad), (pad, pad),
                       (0, 0), (0, 0)), mode='edge')
    ...
    # Do pooling
    output = np.zeros((int(out_height), int(
        out_width), num_channels, batch_size))
    ...
    for i in range(batch_size):
        for j in range(num_channels):
            ...
            if method == 'mean':
                pooled = scipy.signal.convolve(
                    input[:, :, j, i], filter, 'valid') / (filter_size ** 2)
            elif method == 'max':
                pooled = scipy.ndimage.rank_filter(
                    input[:, :, j, i], -1, filter_size)
                # rank_filter generic_filter do their own padding which we
                # don't want, so this line undoes that
                offset = filter_size // 2
                pooled = pooled[offset:offset + h, offset:offset + w]

                # If we are doing backpropagation we need to save where the max
                # values came from
                if backprop:
                    # use generic_filter to avoid getting ties
                    max_idxs = scipy.ndimage.generic_filter(
                        input[:, :, j, i], np.argmax, filter_size)
                    max_idxs = max_idxs[offset:offset + h, offset:offset + w]

                    # The indices returned in max_idxs are all local, so we
                    # need to adjust them to global subscripts
                    max_r, max_c = np.unravel_index(
                        max_idxs.astype('i'), filter.shape)
